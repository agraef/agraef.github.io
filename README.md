...
While I recently started porting some of my software to Lua, because it is more light-weight than Pure and readily available on most platforms, most of the software you'll find here is still written in Pure and/or C/C++.
...
* **New:** [lua-tk](https://github.com/agraef/lua-tk) and [lua-octave](https://github.com/agraef/lua-octave) are ports of my corresponding Pure modules, providing Lua bindings for [Tcl/Tk](https://www.tcl.tk/) and [Octave](http://www.octave.org/). I often use these for my GUI and numeric calculation needs, and since I couldn't find any suitable Lua modules in the usual places, I wrote my own. Both modules can be installed using either [luarocks](https://luarocks.org/) or a plain old `make install`. You will hopefully find these useful if you're developing in Lua, but need to quickly knock out the occasional GUI (Tcl/Tk makes this easy, especially when used with [Gnocl](https://dr-baum.net/gnocl/), see below), or perform some advanced numeric calculations using Octave's extensive numeric library. They are also required for my scale program, see below.
...
* [pd-faustgen2](https://github.com/agraef/pd-faustgen) or, as we call it, the "Faust compiler in a box", is an extensive update of Pierre Guillot's pd-faustgen which brings its feature set up to par with pd-faust as well Grame's faustgen for Max. This is an alternative Faust plugin for Pd which offers some important advantages over pd-faust, most notably JIT compilation of Faust programs using Faust's LLVM backend, and support for recent LLVM versions. If you're starting out with using Faust in Pd, this is the plugin we recommend.
...
* **New:** [Scale](https://github.com/agraef/scale-lua) is my graphical musical scale visualization and rationalization program. Like the Raptor patch from above, it is based on the mathematical music theories of the contemporary composer and pioneer of computer music [Clarence Barlow](https://en.wikipedia.org/wiki/Clarence_Barlow). The original version of this program was written in Pure, but in recent years this has been suffering from severe incompatibilities with modern Mesa 3D library implementations (Pure and Mesa use different, and incompatible, LLVM versions, making them crash). So I ported it over to Lua, which gets rid of these problems. Note that the scale program reads and writes files in Manuel Op de Coul's [Scala](https://www.huygens-fokker.org/scala/) format, thus you can use it with Scala's extensive [scale archive](https://www.huygens-fokker.org/scala/downloads.html#scales). It can also export scales as sysex files in the MIDI Tuning Standard (MTS) format, just like the sclsyx program discussed below. You can use these sysex files to tune MTS-compatible synthesizers such as [Fluidsynth](http://www.fluidsynth.org/).
