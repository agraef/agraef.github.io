...
* **New:** [mdnsbrowser](https://github.com/agraef/mdnsbrowser) is a [Zeroconf](http://en.wikipedia.org/wiki/Zero-configuration_networking) external for Pd written in Pure, which lets you discover and publish services in the local network, so that your Pd patches can establish network connections in an automatic fashion. This is an update for my (outdated) [pd-mdnsbrowser](https://bitbucket.org/agraef/pd-mdnsbrowser) package which was written in Pure. The new version is now written in Lua for easier installation, and thus requires pd-lua to run (see above). Prebuilt binaries for ARM/Intel Macs and Windows are available.
